 ...
 */
 ...
 define('PNG_TEMPLATE_WU', "./modelold.jpg");//无水印模板文件
define('PNG_FONT',"./msyh.ttc" );//字体


define('IMG', serialize(array(0,0,444,800)));
define('PNG_TOUMING',serialize(array(0,397,444,473))); //透明大小
define('PNG_PIC_RECT', serialize(array(0,0,444,473)));//图片、、一个矩行的左上角和右下角

define('PNG_QR_RECT', serialize(array(53,575,210,729)));//二维码 这里可以调节二维码大小，第1,2个数据是左上角的x，y。第3,4个数据是右下角的x，y。eg：往上增大就改第二个数据

define('PNG_TITLE_TEXT', serialize(array(20,397,424,473)));//标题
define('PNG_PRICE_TEXT', serialize(array(269,598,350,608)));//原价
define('PNG_COUPON_PRICE_TEXT', serialize(array(379,598,444,608)));//优惠后价
...
// 		$coupon_url = str_replace('&tmp;','', $coupon_url);
// 		dump($coupon_url);
// 		echo $coupon_url;
...
//			$object->png($item['quan_link'],false, $errorCorrectionLevel, $matrixPointSize, 2);
    ...
    private function drawText($dest,$text,$font,$size,$color,$text_rect,$alignment) {
	...
	$this->drawText($imge,$dtitle,PNG_FONT,14,$color,$rect,DT_VCENTER|DT_CENTER);//写入标题

	$color = imagecolorallocate($imge, 162, 26, 48);
	...
	$this->drawText($imge,number_format($coupon_price,1),PNG_FONT,14,$color,$rect,DT_LEFT|DT_VCENTER);//写入优惠价格
	...
	$this->drawText($imge,$price-$coupon_price,PNG_FONT,30,$color,$rect,DT_LEFT|DT_VCENTER);
