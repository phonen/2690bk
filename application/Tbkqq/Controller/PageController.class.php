...
*/
	...
	public function item_qq_new(){
		...
		if($proxys) {
			foreach ($proxys as $proxy) {
				$proxyid = substr($proxy['proxy'],strlen(C('SITE_APPNAME')));
				$json_url .= $proxyid . "." . $domain . "|" . $proxy['qqgroup'] . ",";
			}
		}
		else {
			$proxys = M("TbkqqProxyQqgrp")->where(array("sendqq" => $qq, "qqstatus" => 1))->select();
			if ($proxys) {
				foreach ($proxys as $proxy) {
					$json_url .= "www." . $domain . "|" . $proxy['qqgroup'] . ",";
				}
			}
		}
		echo $json_url;
...
//		if($group != '' && $qq != ''){
	...
	public function qqgrp_json(){
		...
		if($proxys) {
			foreach ($proxys as $proxy) {
				$proxyid = substr($proxy['proxy'],strlen(C('SITE_APPNAME')));
				$json_arr[] = array("qqgrp"=>$proxy['qqgroup'],"site"=>$proxyid . "." . $domain);
			}
		}
		else {
			$proxys = M("TbkqqProxyQqgrp")->where(array("sendqq" => $qq, "qqstatus" => 1))->select();
			if ($proxys) {
				foreach ($proxys as $proxy) {
					...
					$json_arr[] = array("qqgrp"=>$proxy['qqgroup'],"site"=>"www." . $domain);
				}
			}
		}
		...
		if($send != ''){
			...
			if($t == 'wx'){
				...
				if($proxys) {
					foreach ($proxys as $proxy) {
						$proxyid = substr($proxy['proxy'],strlen(C('SITE_APPNAME')));
						$proxy_arr[] = array("group"=>$proxy['wxgroup'],"site"=>$proxyid . "." . $domain);
					}
				}
			}
			else {
				...
				if($proxys) {
					foreach ($proxys as $proxy) {
						$proxyid = substr($proxy['proxy'],strlen(C('SITE_APPNAME')));
						$proxy_arr[] = array("group"=>$proxy['qqgroup'],"site"=>$proxyid . "." . $domain);
					}
				}
				else {
					...
					if ($proxys) {
						foreach ($proxys as $proxy) {
							$proxy_arr[] = array("qqgrp"=>$proxy['qqgroup'],"site"=>"www." . $domain);
						}
					}
				}
			}
...
长按复制这条消息，打开→手机淘宝→即可领卷下单";
			...
			if($proxy){
				$item = M("TbkqqTaokeItem")->where(array("no"=>$proxy['wxno'],"status"=>"1"))->find();
				if($item){
					if($item['imgmemo'] == '')		$imgurl = $item['img']  . "_290x290.jpg";
					else {
						$preg = '/<img.*?src=[\"|\']?(.*?)[\"|\']?\s.*?>/i';
...
$i++;
	...
	public function test_taotoken(){
		$iid = $_GET['iid'];
		$proxy = M("TbkqqTaokeMedia")->where(array("proxy"=>"taotehui001"))->order("rand()")->find();
		$taoke_model = M('TbkItem','cmf_','DB_DATAOKE');
		$item = $taoke_model->where(array("iid"=>$iid))->find();
		if($item){
			$data = get_url_data($item['quan_link']);
			if($data['activity_id'] == "")$quan_id = $data['activityId'];
			else $quan_id = $data['activity_id'];
			$data = array();
			$data['url'] = "http://uland.taobao.com/coupon/edetail?activityId=" .$quan_id ."&pid=" . $proxy['pid'] ."&itemId=" . $item['iid'] ."&src=qhkj_dtkp&dx=1";
			$data['logo'] = $item['img'];
			$data['text'] = $item['item'];
echo "abc";
