...
class AdminProblemController extends AdminbaseController {
	protected $problem_model;
	protected $users_model;
	
	function _initialize() {
		parent::_initialize();
		$this->problem_model = D("Portal/Problem");
	...
	function add_post(){
		if (IS_POST) {
			$_POST['post']['add_time']=date("Y-m-d H:i:s",time());
			$_POST['post']['author']=get_current_admin_id();
			$article=I("post.post");						$article['status']="使用中";
			$article['content']=htmlspecialchars_decode($article['post_content']);
			$result=$this->problem_model->add($article);
			if ($result) {
				admin_record_operation(310,$article['title']);
				$this->success("添加成功！");
			} else {
				$this->error("添加失败！");
			}
			 
		}else{						$this->error("请填写内容");					}
	}
	
	public function edit(){
		$id=  intval(I("get.id"));
		$post=$this->problem_model		->alias("a")		->join(C( 'DB_PREFIX' )."users b ON a.author=b.id")		->where("a.id=$id")		->find();
	...
	public function edit_post(){
		if (IS_POST) {
			...
			if ($result!==false) {				admin_record_operation(311,$article['title']);			
				$this->success("保存成功！");
			} else {
				$this->error("保存失败！");
			}
