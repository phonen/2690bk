...
class LoginController extends HomebaseController {
	...
	//登出
    public function official_accounts_logout(){
        session("user",null);
        redirect(__ROOT__."/?a=official_accounts");
    }
	function active(){
		$this->check_login();
		$this->display(":active");
    ...
    private function _do_mobile_login(){
        ...
        if(!empty($result)){
            if(sp_compare_password($password, $result['user_pass'])){
				if(I("post.avoid_login","")=="avoid_login"){
					//设置session15天灭活
					$this->start_session(3600*60*24*15);
				}
                        ...
                        if($result){//本应用已经有这个用户
                            if(sp_compare_password($password, $result['user_pass'])){//本应用已经有这个用户,且密码正确，同步用户
                                ...
                                if($uc_uid2<0){
                                    ...
                                    //$this->error("同步用户失败--".$uc_register_errors[$uc_uid2]);
	                                $this->send_mes("同步用户失败--".$uc_register_errors[$uc_uid2]);
                                    return;
