$(function(){
    ...
    function resetleft(obj) {
        ...
        if(parW + parL + selfW + 170 >= $(document).width()){
    ...
    var copyFun = function (obj){
        ...
        if(obj.find('.copyText').length>0){
            ...
            var url='/?a=gettpl&id='+ obj.siblings('.go_info').data('gid') ;
            $.ajax({
                type:'get',
                url:url,
                cache:false,
                success:function(res){
                    if(res){
                		$.ajax({
			                type:'post',
			                url:'/?a=ajax_getmes',
			                data:'iid='+obj.siblings('.go_info').data('gid'),
			                cache:false,
			                success:function(response){
			                	responses=eval("("+response+")");
			                	if(responses.mes_status==1){
			                		if(responses.dtitle){
			                			responses.title=responses.dtitle;
			                		}
			                		obj.append('<div class="copyText">'+res+'<div class="textContent">【天猫】'
			                		+responses.title+'<br>【价格】原价'
			                		+responses.price+'元  券后'
			                		+responses.coupon_price+'元包邮<br>【推荐理由】'
			                		+responses.intro+'<br>---------分割线--------<br>长按复制这条信息 '
			                		+responses.token+' ,打开【手机淘宝家乡版】即可领券并下单</div></div>');
